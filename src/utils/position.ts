...
export function getChangedPosition(
	start: Position,
	edit: TextEdit
...
export function adjustPosition(pos: Position, edit: TextEdit): Position {
	...
	let removed =
		start.line == end.line && newLines.length == 1 ? end.character - start.character : end.character
...
export function editRange(range: Range, text: string, edit: TextEdit): string {
	...
	let character =
		start.line == range.start.line ? start.character - range.start.character : start.character
